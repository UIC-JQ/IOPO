...
class LSTM_Model(nn.Module):
    def __init__(
        ...
        self.data_X = torch.zeros((self.Memory_size, self.input_feature_size), dtype=torch.float32).to(torch.device('cuda'))
        self.data_Y = torch.zeros((self.Memory_size, self.data_config.user_number), dtype=torch.float32).to(torch.device('cuda'))
    ...
    def train(self):
        ...
        overload_factor = torch.ones((self.batch_size, self.data_config.uav_number), requires_grad=False).to(torch.device('cuda'))

        # 1st stage: encode:
        x_each_person_features = self.encoder(x_each_person_features)
        dec_h = torch.zeros((self.batch_size, self.hidden_feature_size)).to(torch.device('cuda'))
        dec_c = torch.zeros((self.batch_size, self.hidden_feature_size)).to(torch.device('cuda'))
    ...
    def generate_answer(self, x, data_config):
        ...
        overload_factor = torch.ones((1, data_config.uav_number)).to(torch.device('cuda'))

        # 1st stage: encode:
        x_each_person_features = self.encoder(x_each_person_features)
        dec_h = torch.zeros(1, self.hidden_feature_size).to(torch.device('cuda'))
        dec_c = torch.zeros(1, self.hidden_feature_size).to(torch.device('cuda'))
