...
def generate_better_allocate_plan_KMN(nn_ans,
    ...
    threshold_0 = torch.full(flatten_prob.shape, fill_value=threshold_p, dtype=torch.float32).to(torch.device('cuda'))
    ...
    return torch.scalar_tensor(eng_cost), torch.Tensor(final_allocation_plan)